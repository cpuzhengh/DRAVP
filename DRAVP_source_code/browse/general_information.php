    ...
    <div class="row mt40">
                ...
                <div class="panel panel-info" style="position: fixed;">
                    ...
                    <div class="panel-footer">
                        <ul class="nav nav-pills nav-stacked">
                            ...
                            <li><a href="#Origin Information">Origin Information</a></li>
        ...
        <!--general information-->
            ...
            <div style="padding: 12px;background-color: #f7f7f7;border: 1px solid #ddd;border-radius: 12px;">
                  ...
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Taxon ID</span>&nbsp;
                     <?php 
                    if($row['Taxon_ID'] == "None") {
                         echo htmlspecialchars($row['Taxon_ID']);
                     } else {
                          $pieces = explode("##", $row['Taxon_ID']);
                          $count = count($pieces);
                          for($i = 0; $i < $count; $i++) {
                              $taxonId = trim($pieces[$i]);
                              if(!empty($taxonId)) {
                                  echo '<a href="https://www.uniprot.org/taxonomy/'.urlencode($taxonId).'">'.htmlspecialchars($taxonId).'</a>&nbsp;';
                              }
                          }
                     }
                     ?>    
                  ...
                  <?php 
                    if (!empty($row['Other_link'])){
                        echo '<p><span style="color: #4478b1;font-size:18px;">Other Link</span>&nbsp; <a href="patent_information.php?id='.$row['Other_link'].'" >'.$row['Other_link'].'</a></p>';
                    }
                  ?>
                  <p>
            ...
            <br><br>
            <p id="Origin Information" name="Origin Information">
                <strong style="font-size: 22px">Origin Information</strong>
            </p>
            <hr style="border-top: 1px solid #ddd;border-bottom: 1px dotted #fff;">
            <div style="padding: 12px;background-color: #f7f7f7;border: 1px solid #ddd;border-radius: 12px;">
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Gene Name/ID</span>&nbsp; <?php echo $row['Gene_Name_ID']?>
                  </p>
                  <p>
                      <span style="color: #4478b1;font-size:18px;">GenBank</span>&nbsp; <?php echo $row['GenBank']?>
                  </p>
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Amino Acid position</span>&nbsp; <?php echo $row['Amino acid position']?>
                  </p>
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Domain Accession ID</span>&nbsp;
                     <?php 
                       if($row['Domain_Accession_ID'] == "Not Available") {
                         echo htmlspecialchars($row['Domain_Accession_ID']);
                     } else {
                          $pieces = explode("##", $row['Domain_Accession_ID']);
                          $count = count($pieces);
                          for($i = 0; $i < $count; $i++) {
                              $domainId = trim($pieces[$i]);
                              if(!empty($domainId)) {
                                  echo '<a href="https://www.ncbi.nlm.nih.gov/Structure/cdd/cddsrv.cgi?ascbin=8&maxaln=10&seltype=2&uid='.urlencode($domainId).'">'.htmlspecialchars($domainId).'</a>&nbsp;';
                              }
                          }
                     }
                     ?>  
                  ...
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Nucleotide sequence ID</span>&nbsp; 
                     <?php 
                       if($row['Nucleotide_sequence_ID'] == "Not Available") {
                         echo htmlspecialchars($row['Nucleotide_sequence_ID']);
                     } else {
                          $pieces = explode("##", $row['Nucleotide_sequence_ID']);
                          $count = count($pieces);
                          for($i = 0; $i < $count; $i++) {
                              $nucleotideId = trim($pieces[$i]);
                              if(!empty($nucleotideId)) {
                                  echo '<a href="https://www.ncbi.nlm.nih.gov/nuccore/'.urlencode($nucleotideId).'">'.htmlspecialchars($nucleotideId).'</a>&nbsp;';
                              }
                          }
                     }
                     ?>  
                  ...
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Molecular Type</span>&nbsp; <?php echo $row['Molecular_Type']?>
                  </p>
                  <p>
                      <span style="color: #4478b1;font-size:18px;">Chromosomal Position</span>&nbsp; <?php echo $row['Chromosomal position']?>
                  </p>  
            ...
            <div style="padding: 12px;background-color: #f7f7f7;border: 1px solid #ddd;border-radius: 12px;">
                ...
                <p>
                    ...
                    <?php 
                    if ($row['Predicted_structure_ID']=='No predicted structure available'){
                        echo $row['Predicted_structure_ID'];
                    }else{
                        echo '<a href="http://dravp.cpu-bioinfor.org/Structure/download.php?filename=./predicted_structure/'.$row['Predicted_structure_ID'].'.pdb" >'.$row['Predicted_structure_ID'].'</a>';
            ...
            <div style="padding: 12px;background-color: #f7f7f7;border: 1px solid #ddd;border-radius: 12px;">
                ...
                <style>
				        ...
				        <script>
				            ...
				            chart.dataProvider = [{acid:"A",number:<?php echo "{$physical_info['Alanine']}"; ?>},{acid:"R",number:<?php echo "{$physical_info['Arginine']}"; ?>},
				                    {acid:"N",number:<?php echo "{$physical_info['Asparagine']}"; ?>},
				                    {acid:"D",number:<?php echo "{$physical_info['Aspartic_Acid']}"; ?>},
				                    {acid:"C",number:<?php echo "{$physical_info['Cysteine']}"; ?>},
				                    {acid:"E",number:<?php echo "{$physical_info['Glutamic_Acid']}"; ?>},
				                    {acid:"Q",number:<?php echo "{$physical_info['Glutamine']}"; ?>},
				                    {acid:"G",number:<?php echo "{$physical_info['Glycine']}"; ?>},
				                    {acid:"H",number:<?php echo "{$physical_info['Histidine']}"; ?>},
				                    {acid:"I",number:<?php echo "{$physical_info['Isoleucine']}"; ?>},
				                    {acid:"L",number:<?php echo "{$physical_info['Leucine']}"; ?>},
				                    {acid:"K",number:<?php echo "{$physical_info['Lysine']}"; ?>},
				                    {acid:"M",number:<?php echo "{$physical_info['Methionine']}"; ?>},
				                    {acid:"F",number:<?php echo "{$physical_info['Phenylalanine']}"; ?>},
				                    {acid:"P",number:<?php echo "{$physical_info['Proline']}"; ?>},
				                    {acid:"S",number:<?php echo "{$physical_info['Serine']}"; ?>},
				                    {acid:"T",number:<?php echo "{$physical_info['Threonine']}"; ?>},
				                    {acid:"W",number:<?php echo "{$physical_info['Tryptophan']}"; ?>},
				                    {acid:"Y",number:<?php echo "{$physical_info['Tyrosine']}"; ?>},
				                    {acid:"V",number:<?php echo "{$physical_info['Valine']}"; ?>}];
